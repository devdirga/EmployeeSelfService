...
namespace KANO.ESS.Areas.ESS.Controllers
{
    ...
    public class AbsenceController : Controller
    {
        ...
        public IActionResult GetEntityList()
        {
            ...
            //var client = new Client(Configuration);
            //var request = new Request($"{ApiEntity}list?&skip={skip}&limit={limit}&search={search}", Method.GET, "Authorization", bearerAuth);
            //var bearerAuth = "Bearer ";
            //if (Request.Headers.TryGetValue("Authorization", out StringValues authToken))
            //{
            //    bearerAuth = authToken;
            ...
            //request.Self.AddHeader("Authorization", bearerAuth);
            //EntityMap
            //var response = new Client(Configuration).Execute(new Request($"{ApiEntity}list?&skip={skip}&limit={limit}&search={search}", Method.GET, "Authorization", bearerAuth));
            return new ApiResult<List<EntityMap>>(
                JsonConvert.DeserializeObject<ApiResult<List<EntityMap>>.Result>(
                    new Client(Configuration).Execute(new Request($"{ApiEntity}list?&skip={skip}&limit={limit}&search={search}", Method.GET, "Authorization", bearerAuth)).Content));
